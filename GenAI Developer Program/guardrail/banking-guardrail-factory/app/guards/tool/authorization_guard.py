...
class AuthorizationGuard(BaseGuardrail):
    ...
    def check(self, context: dict) -> GuardrailResult:
        ...
        return self.check_account_access(user_id=user_id, account_id=account_id)

    def check_account_access(self, user_id: str, account_id: str) -> GuardrailResult:

        # Mock banking ownership data for learning
        ...
        if account_id not in allowed_accounts:
            return GuardrailResult(
                ...
                reason=(
                    ...
                    f"to access account {account_id}."
                ),
            )
        ...
        return GuardrailResult(
            ...
            guardrail=self.name,
            reason=(
                f"User {user_id} is authorized " f"to access account {account_id}."
            ),
        )
